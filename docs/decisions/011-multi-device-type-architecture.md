...
await control_client.async_set_temperature(ata_id, temp)  # Handles retry (dedup since removed, ADR-026)
...
├── control_client_base.py       # Shared retry infrastructure (dedup since removed, ADR-026)
...
2. **State deduplication**: Skip API calls when values unchanged (70% reduction in typical scenes) [removed, see [ADR-026](026-remove-control-write-dedup.md); the layer now applies each accepted write to the coordinator's copy of the unit instead]
