# MELCloud Home v1.1 - Simplified Scope

**Priority:** High
**Estimated Effort:** 4 hours (reduced from 7)
**Focus:** Real-time updates + essential sensors only

---

## Core Value Proposition

**v1.0 Problem:** 60-second polling delay for state updates
**v1.1 Solution:** WebSocket for instant updates (< 1 second)

---

## What's IN v1.1 ✅

### 1. WebSocket Real-Time Updates (2 hours)

**Goal:** Instant climate entity updates via WebSocket

**Implementation:**
- WebSocket manager (`websocket.py`)
- Connection to `wss://ws.melcloudhome.com`
- Parse 3 message types:
  - `unitStateChanged` → Update climate entity
  - `unitWifiSignalChanged` → Update WiFi sensor
  - `unitErrorStateChanged` → Update error sensor
- Exponential backoff reconnection
- **Fallback:** If WebSocket fails after 6 retries → stay on polling

**Coordinator Changes:**
- Keep existing 60s polling as **backup only**
- WebSocket updates trigger `async_set_updated_data()`
- If WebSocket disconnected > 5 minutes → resume polling
- Simple: One primary update source at a time (no concurrent handling)

**Architecture:**
```
WebSocket Active:  WebSocket → Coordinator → Entities
                   (polling suspended)

WebSocket Failed:  REST Polling → Coordinator → Entities
                   (resume 60s polling)
```

### 2. WiFi Signal Sensor (30 minutes)

**Entity:** `sensor.{building}_{room}_wifi_signal`
**Device Class:** `signal_strength`
**Unit:** dBm
**Update:** WebSocket `unitWifiSignalChanged`
**Attributes:**
- `rssi`: -53
- `quality`: "excellent" | "good" | "fair" | "poor"

**Value:** Monitor connectivity health, troubleshoot connection issues

### 3. Error State Binary Sensor (30 minutes)

**Entity:** `binary_sensor.{building}_{room}_error`
**Device Class:** `problem`
**State:** `on` (error) | `off` (normal)
**Update:** WebSocket `unitErrorStateChanged`
**Attributes:**
- `error_code`: "E01"
- `error_message`: "Filter needs cleaning" (if available)

**Value:** Instant error notifications, proactive maintenance

### 4. Integration Icon (15 minutes)
...
### 5. Basic Diagnostics (30 minutes)
...
```json
{
  ...
  "websocket": {
    "enabled": true,
    "state": "connected" | "disconnected" | "fallback_polling",
    "uptime_seconds": 3600,
...
### 6. Entity Naming Fix (15 minutes)

**Current:** `sensor.home_bedroom_heatpump_wifi_signal`
**New:** `sensor.home_bedroom_wifi_signal`

**Changes:**
- Remove `heatpump_` from all entity IDs
- Cleaner, less redundant (device name already includes "Heat Pump")

**Entities:**
- `climate.home_bedroom` (existing, no change)
- `sensor.home_bedroom_wifi_signal` ✅
- `binary_sensor.home_bedroom_error` ✅
...
**NEW:**
- `custom_components/melcloudhome/websocket.py` (WebSocket manager)
- `custom_components/melcloudhome/sensor.py` (WiFi sensor)
- `custom_components/melcloudhome/binary_sensor.py` (Error sensor)
...
**MODIFIED:**
- `custom_components/melcloudhome/coordinator.py` (WebSocket integration)
- `custom_components/melcloudhome/const.py` (WebSocket constants)
- `custom_components/melcloudhome/manifest.json` (Add platforms)
- `custom_components/melcloudhome/climate.py` (Entity naming fix)
