...
class TestOutdoorTemperatureVCR:
    """Test outdoor temperature API with VCR cassettes."""

    @freeze_time("2026-02-03 12:30:00", real_asyncio=True)
    @pytest.mark.vcr()
    @pytest.mark.asyncio
    async def test_get_outdoor_temperature(self, credentials: tuple[str, str]) -> None:
        ...
        result = await client.get_outdoor_temperature(unit.id)
